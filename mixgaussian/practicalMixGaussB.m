...
for (cData =1:nData)
    ...
    data(:,cData) = sqrt(mixGauss.cov(1,1,h))*randn(1)+mixGauss.mean(h);
end;
...
mixGaussEst.d = 1;
mixGaussEst.k = k;
mixGaussEst.weight = (1/k)*ones(1,k);%prior
...
for (cIter = 1:nIter)
   ...
   for (cData = 1:nData)
        ...
        rd = 0;
        for (i = 1:k)
            rd = rd+getGaussProb(data(cData),mixGaussEst.mean(i),mixGaussEst.cov(i))*mixGaussEst.weight(i);
        end
        for (i = 1:k)
            rn = getGaussProb(data(cData),mixGaussEst.mean(i),mixGaussEst.cov(i))*mixGaussEst.weight(i);
            responsibilities(i,cData) = rn/rd;
        end
   end;
   ...
   for (cGauss = 1:k) 
        ...
        mixGaussEst.weight(cGauss) = sum(responsibilities(cGauss,:))/sum(responsibilities(:)); 
        ...
        C = sum(responsibilities(cGauss,:).*(data-mixGaussEst.mean(cGauss))*(data-mixGaussEst.mean(cGauss))');
        mixGaussEst.cov(1,1,cGauss) = C/sum(responsibilities(cGauss,:));
   end;
...
for(cData = 1:nData)
    ...
    for (i=1:mixGaussEst.k)
        like = like+ getGaussProb(thisData,mixGaussEst.mean(i),mixGaussEst.cov(1,1,i))*mixGaussEst.weight(i);
    end
...
for(cData = 1:nData)
    ...
    boundValue = 0;
    for (i=1:mixGaussEst.k)
        boundValue= boundValue+thisQ(i)*(getMixGaussLogLike(thisData,mixGaussEst)-log(thisQ(i)));
    end
    %add to total log like
