...
like = 1/(sqrt((2*pi)^length(data)*det(gaussCov)))*exp(-1/2*(data-gaussMean)'*inv(gaussCov)*(data-gaussMean));
...
%covData = cov(transpose(data));
C = zeros(3,3);
for i = 1:nData
    C = C +(data(:,i)-meanData)*((data(:,i)-meanData)');
end
covData = C/nData;
